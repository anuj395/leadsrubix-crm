#!/usr/bin/env node
/**
 * scripts/run_full_system_audit.js
...
async function main() {
  ...
  await runTest('Authentication', 'SuperAdmin Login & Credential Verification', async () => {
    const res = await authService.login('superadmin@leadsrubix.com', 'SuperPassword@123');
  ...
  await runTest('Authentication', 'Invalid Password Rejection with HTTP 401', async () => {
    try {
      await authService.login('superadmin@leadsrubix.com', 'WrongPassword!999');
  ...
  await runTest('Dynamic Industries', 'Retrieve Active Industries List', async () => {
    const list = await industryService.listActive();
...
---
*Generated automatically by \`scripts/run_full_system_audit.js\`*
`;
