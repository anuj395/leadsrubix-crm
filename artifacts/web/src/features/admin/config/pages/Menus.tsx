...
export default function AdminMenusPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setToast({ open: true, msg: e?.response?.data?.message ?? 'Failed to load sidebar menus', sev: 'error' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const submit = async () => {
    ...
    try {
      ...
      setDialogOpen(false)
      setToast({ open: true, msg: 'Menu configuration saved successfully', sev: 'success' })
      await refresh()
    } catch (e: any) {
  ...
  const remove = async (row: SidebarMenuRecord) => {
    confirmDelete({
      ...
      message: `Delete sidebar menu "${row.name}"? This change applies strictly to your organization.`,
      onConfirm: async () => {
        try {
          await deleteMenuRecord(row._id)
          setToast({ open: true, msg: 'Menu deleted', sev: 'success' })
          await refresh()
        } catch (e: any) {
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3 }, height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <AppCard
        title="Organization Sidebar Menus"
        subtitle="Manage and customize navigation entries for your organization in complete tenant isolation."
        action={
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{form._id ? 'Edit Menu' : 'New Custom Menu'}</DialogTitle>
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <TextField
              ...
              helperText="Unique key (e.g. leads.contacts)"
              disabled={!!form._id}
              fullWidth
            />
            <TextField
            ...
            <TextField
              ...
              fullWidth
              helperText="Grouping key (e.g. leads, configuration)"
            />
            <FormControlLabel
              control={
                <Switch
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={submit} disabled={saving}>
            {saving ? <CircularProgress size={18} /> : 'Save Menu'}
          </Button>
        </DialogActions>
      </Dialog>
