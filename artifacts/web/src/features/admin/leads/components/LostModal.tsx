    ...
    try {
      ...
      await Promise.all(allTasks.map((t: any) => {
        ...
        return api.put(`tasks/${t._id}`, {
          ...
          status: nextStatus,
          stage: 'LOST',
          notes: (t.notes ? `${t.notes}\n` : '') + `[Lead Status: Lost - ${values.lostReason || 'Closed Lost'}]`
        }).catch(() => null)
      }))

      // 3. Save comprehensive audit note to contact timeline
      const noteContent = String(taskFields.notes || '').trim()
      const reasonLabel = values.lostReason === 'Other' && values.otherLostReason ? values.otherLostReason : values.lostReason
      const auditNote = `[Status: Lost - ${reasonLabel || 'Closed Lost'}]${noteContent ? ` Details: ${noteContent}` : ''}`
      ...
      setToast({ open: true, msg: 'Lead marked as Lost with activity preserved!', sev: 'success' })
      setTimeout(() => {
        onSuccess()
        onClose()
      }, 800)
